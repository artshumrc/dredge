...
def generate_client_source(config: DredgeConfig) -> str:
    ...
    sort_fields = _sort_field_union(config)
...
def _sort_field_union(config: DredgeConfig) -> str:
    """The `documents` columns the engine will order by, for this site.

    Store Fields live on the same table but the engine rejects them as a sort
    field, and array Facets have no column at all, so neither is offered.
    """
    fields = _unique(
        ("id", "url", "title", "description")
        + tuple(facet.name for facet in config.scalar_facets)
    )
    return "\n".join(f"  | {json.dumps(field)}" for field in fields)
